...
docker run -h ${PHP_NODE_NAME} -d --name php_app \
...
docker run -h ${NODE_NODE_NAME} -d --name nodejs_app \
	-e ACCOUNT_NAME=${ACCOUNT_NAME} -e ACCESS_KEY=${ACCESS_KEY} -e SSL=${SSL} \
...
echo -n "python_app: "
docker run -h ${PY_NODE_NAME} -d --name python_app \
	-e ACCOUNT_NAME=${ACCOUNT_NAME} -e ACCESS_KEY=${ACCESS_KEY} -e SSL=${SSL}\
	-e CONTROLLER=${CONTR_HOST} -e APPD_PORT=${CONTR_PORT} \
	-e APP_NAME=${APP_NAME} -e TIER_NAME=${PY_TIER_NAME} -e NODE_NAME=${PY_NODE_NAME} \
	-p 9000:9000 appdynamics/python-app:latest
...
echo -n "Starting java_app"
docker run -h ${JAVA_NODE_NAME} -d --name java_app -p 8080:8080 \
        -e ACCOUNT_NAME=${ACCOUNT_NAME} -e ACCESS_KEY=${ACCESS_KEY} -e SSL=${SSL} \
        -e APP_NAME=${APP_NAME} -e JAVA_TIER_NAME=${JAVA_TIER_NAME} -e JAVA_NODE_NAME=${JAVA_NODE_NAME} \
        -e CONTROLLER=${CONTR_HOST} -e APPD_PORT=${CONTR_PORT} \
